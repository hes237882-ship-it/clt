/* 워드맥스5 단어테스트 - 단일 파일 PWA(프론트 JS)
   - Day 선택(모달) / 퀴즈(4지선다) / 플래시카드 / 오답만
   - 발음: Web Speech API (speechSynthesis)
   - data.json 필요: { "Day1":[{word,meaning},...], "Day2":[...] ... }
*/
...
const state = {
  ...
  list: [],              // current questions (shuffled)
  idx: 0,
  correct: 0,
...
function normalizeDay(dayKey) {
  // "day1" -> "Day1"
  return String(dayKey).replace(/^day/i, "Day");
}
...
async function loadData() {
  ...
  const keys = sortDayKeys(Object.keys(state.data || {}));
  if (keys.length) state.day = normalizeDay(keys[0]);
...
function makeChoices(correctMeaning) {
  // NOTE: 현재 state.list 내 meaning에서만 뽑음(동일 day 기준)
      ...
      ${
        wrongWords.length
          ? wrongWords
              .map(
                (w) =>
                  `<span class="pill" style="display:inline-block; margin:4px 6px 0 0;">${w}</span>`
              )
              .join("")
          : "<span class='tiny'>오답이 없습니다 🎉</span>"
      }
    </div>
  `;
...
function toggleWrongOnly() {
  ...
  if (state.onlyWrong) {
    ...
    const raw = (state.data[state.day] || [])
      .filter((x) => state.wrongSet.has(x.word))
      .map((x) => ({ ...x }));
    state.list = shuffle(raw);
  } else {
    $("wrongBtn").textContent = "오답만";
